...
project = res.data[0]
print(f"Project: {project['id']}, status: {project.get('status', 'N/A')}")
...
# get transcripts
transcripts = sb.table("transcripts").select("*").eq("project_id", project["id"]).execute()
print(f"Transcripts count: {len(transcripts.data)}")
